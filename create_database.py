...
def write(code):
    try:
        dat = w.wsd(code, d, 'ipo')
        data = pd.DataFrame(dat.Data, index=dat.Fields, columns=dat.Times).T
        ...
        print('Stock '+code+' does not exist in WIND database!')
        

conn = mysql.connector.connect(host = '10.23.0.2', port=3306, user='root', password='11031103', database='test')   
cursor = conn.cursor()
#
